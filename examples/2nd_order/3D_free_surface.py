"""
Simple 3D example implementing free-surface with the 2nd-order acoustic wave
...
def run(sdf, s_o, nsnaps):
    ...
    bc_list = [dv.Eq(p, 0),  # Zero pressure on free surface
               dv.Eq(p.dx2+p.dy2+p.dy2, 0)]  # Zero laplacian

    if s_o >= 4:
        bc_list.append(dv.Eq(p.dx4 + p.dy4 + p.dz4
...
def main():
    ...
    outfile = append_path("/3D_free_surface_snaps.npy")
