"""
Simple example implementing a free-surface with Fletcher et al's TTI
...
def run(sdf, s_o, nsnaps):
    ...
    rhs_p = v_px**2*h2(p, th) + v_py**2*h1(q, th) \
        + v_sy**2*h1(p, th) - v_sy**2*h1(q, th)
    rhs_q = v_pn**2*h2(p, th) + v_py**2*h1(q, th) \
        - v_sy**2*h2(p, th) + v_sy**2*h2(q, th)
...
def main():
    ...
    outfile = append_path("/2D_tti_fletcher_snaps.npy")
